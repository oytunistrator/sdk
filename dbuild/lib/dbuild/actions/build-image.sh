...
while read line; do
	if [[ ! "$line" == \#* ]]; then
		...
		sz=$(du --apparent-size -sm "${DBUILD_BUILD_DIR}/${disk_name}-${EXPIDUS_VERSION}-${TARGET_ARCH}")
		if [[ "${sz}" != "0" ]]; then
			sz=$((sz + sz / 6))
			...
			if [[ "${part}" == "vfat" ]]; then
				UUID="${blkdev/UUID=/}"
				printf "\x${UUID:7:2}\x${UUID:5:2}\x${UUID:2:2}\x${UUID:0:2}" | dd bs=1 seek=67 count=4 conv=notrunc of="${DBUILD_BUILD_DIR}/${disk_name}-${EXPIDUS_VERSION}-${TARGET_ARCH}.img"
			else
